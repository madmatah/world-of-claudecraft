...
export const ko_KR: EnTranslations = {
  ...
  "probe": {
    ...
    "consent": {
      ...
      "body": "테스트는 창을 몇 개 스스로 열었다 닫으며 약 {minutes}분이 걸립니다. 테스트와 테스트 사이에 화면이 몇 초 동안 비어 보일 수 있지만 정상이며, 문제가 생긴 것이 아닙니다. 게임은 닫아 두고, 노트북은 전원에 연결하고, 결과가 나올 때까지 컴퓨터를 건드리지 마세요.",
      "start": "테스트 시작",
      "cancel": "나중에"
    },
    "progress": {
      ...
      "overall": "테스트 {index}/{total}",
      "between": "테스트 {done}/{total}을(를) 마쳤습니다. 다음 테스트를 시작합니다.",
  ...
  "hudChrome": {
    ...
    "options": {
      ...
      "gpuBackendVerdict": "마지막 백엔드 테스트: {backend}.",
      "gpuBackendVerdictStale": "마지막 백엔드 테스트 결과가 더 이상 이 PC와 일치하지 않습니다. 다시 실행하여 백엔드를 선택하세요.",
      "testBackends": "그래픽 백엔드 테스트",
      "testBackendsNote": "게임을 다시 시작하여 이 PC에서 각 그래픽 백엔드를 측정하는 테스트(몇 분 소요)를 실행하고 자동 설정에 가장 적합한 것을 선택합니다.",
      "probeRequestedTitle": "그래픽 백엔드 테스트를 실행할까요?",
      "probeRequestedBody": "테스트를 하려면 게임을 닫아야 합니다. 지금 테스트로 다시 시작할까요? 현재 세션이 종료됩니다.",
      "probeRequestedConfirm": "테스트로 다시 시작",
      "probeRequestedCancel": "나중에",
