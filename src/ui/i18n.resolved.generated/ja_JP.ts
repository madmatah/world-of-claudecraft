...
export const ja_JP: EnTranslations = {
  ...
  "probe": {
    ...
    "consent": {
      ...
      "body": "テストはいくつかのウィンドウを自動で開いたり閉じたりし、約{minutes}分かかります。テストとテストの間、画面が数秒間何も表示されないことがありますが、異常ではなく、故障でもありません。ゲームは閉じたまま、ノートPCは電源につなぎ、結果が表示されるまでコンピューターに触れないでください。",
      "start": "テストを開始",
      "cancel": "今はしない"
    },
    "progress": {
      ...
      "overall": "テスト {index}/{total}",
      "between": "テスト {done}/{total} が完了しました。次のテストを開始します。",
    ...
    "status": {
      ...
      "capped": "このバックエンドはテスト用シェーダーを時間内にリンクできませんでした。"
    }
  },
  "devCommand": {
  ...
  "hudChrome": {
    ...
    "options": {
      ...
      "gpuBackendVerdict": "前回のバックエンドテスト: {backend}。",
      "gpuBackendVerdictStale": "前回のバックエンドテストの結果はこのPCと一致しなくなりました。もう一度実行してバックエンドを選び直してください。",
      "testBackends": "グラフィックバックエンドをテスト",
      "testBackendsNote": "ゲームを再起動し、このPCで各グラフィックバックエンドを計測するテスト（数分）を行って、自動設定に最適なものを選びます。",
      "probeRequestedTitle": "グラフィックバックエンドのテストを実行しますか？",
      "probeRequestedBody": "テストにはゲームを閉じる必要があります。今すぐテストに再起動しますか？現在のセッションは終了します。",
      "probeRequestedConfirm": "テストに再起動",
      "probeRequestedCancel": "今はしない",
