...
float computeSpecularOcclusion( const in float dotNV, const in float ambientOcclusion, const in float roughness ) {
	...
	#if 0 > 0
		#if defined( SHADOWMAP_TYPE_PCF )
			uniform sampler2DShadow spotShadowMap[ 0 ];
		#else
	...
	#if 0 > 0
		#if defined( SHADOWMAP_TYPE_PCF )
			uniform samplerCubeShadow pointShadowMap[ 0 ];
		#elif defined( SHADOWMAP_TYPE_BASIC )
			uniform samplerCube pointShadowMap[ 0 ];
		#endif
...
void main() {
	...
	diffuseColor *= vColor;
        ...
        if ( uWornTaps > 0.0 && wornCamD < uWornParEnd ) {
          ...
          // under 2 screen pixels; the fade band eases the
          ...
          if ( uWornTaps > 1.0 ) {
            float wornTapW = min( uWornTaps - 1.0, 1.0 );
          ...
          if ( uWornTaps > 2.0 ) {
            float wornTapW = min( uWornTaps - 2.0, 1.0 );
            wornH = wornTriR( uWornDisp,
            wornP + wornV * ( wornH * uWornParallaxAmp ), wornW, wornAxis ) - uWornDispCenter;
          wornHAcc += wornH * wornTapW;
          wornHN += wornTapW;
          ...
          if ( uWornTaps > 3.0 ) {
            float wornTapW = min( uWornTaps - 3.0, 1.0 );
            wornH = wornTriR( uWornDisp,
            wornP + wornV * ( wornH * uWornParallaxAmp ), wornW, wornAxis ) - uWornDispCenter;
          wornHAcc += wornH * wornTapW;
          wornHN += wornTapW;
        ...
        diffuseColor.rgb *= 1.0 + wornHShade * uWornHeightShade * wornCellK;
...
diffuseColor.a = ( ( floatBitsToUint( diffuseColor.a ) & 0x7f800000u ) == 0x7f800000u ) ? 0.0 : diffuseColor.a;
outgoingLight.x = ( ( floatBitsToUint( outgoingLight.x ) & 0x7f800000u ) == 0x7f800000u ) ? 0.0 : outgoingLight.x;
outgoingLight.y = ( ( floatBitsToUint( outgoingLight.y ) & 0x7f800000u ) == 0x7f800000u ) ? 0.0 : outgoingLight.y;
outgoingLight.z = ( ( floatBitsToUint( outgoingLight.z ) & 0x7f800000u ) == 0x7f800000u ) ? 0.0 : outgoingLight.z;
...
wocFogNanGuard.x = ( ( floatBitsToUint( wocFogNanGuard.x ) & 0x7f800000u ) == 0x7f800000u ) ? 0.0 : wocFogNanGuard.x;
wocFogNanGuard.y = ( ( floatBitsToUint( wocFogNanGuard.y ) & 0x7f800000u ) == 0x7f800000u ) ? 0.0 : wocFogNanGuard.y;
wocFogNanGuard.z = ( ( floatBitsToUint( wocFogNanGuard.z ) & 0x7f800000u ) == 0x7f800000u ) ? 0.0 : wocFogNanGuard.z;
